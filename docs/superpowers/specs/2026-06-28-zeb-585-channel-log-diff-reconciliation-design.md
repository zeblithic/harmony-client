...
**Encryption (Jake's call):** encrypt the vector with the **same channel symmetric key the reply packets already use**, AEAD. Both sides are authorized community peers holding that key (the responder must hold it to encrypt reply packets). Benefits: (1) does not widen cleartext metadata beyond the status-quo key-expr (`cid`/`ch`/scalar-`since`); (2) AEAD authenticity gives the **malformed/tampered-vector fallback for free** — decrypt failure → treat as no payload → scalar path. Nonce derivation reuses the reply-packet scheme. *Plan-time:* locate the channel reply-packet key + nonce source at query-build time on the requester and at query-serve time on the responder; if the key is not conveniently available at query-build time, fall back to **no payload** (scalar path) rather than sending cleartext.

## A.5 Cap-before-alloc (security)

Mirror the established precedent at `event_loop.rs:5626` (the `MAX_PAIRING_WIRE_BYTES` check that "MUST run BEFORE the heap allocation," CodeRabbit PR #63): introduce `MAX_WATERMARK_VECTOR_BYTES`, checked on the payload **bytes view before decrypt/decode**. Over cap → ignore the payload, serve via the key scalar. On the **requester** side: if the encoded vector would exceed the cap, do not attach it (degrade to scalar + floor) — simplest correct degradation; top-N-most-recent-device subsetting is a possible future refinement (YAGNI for this PR). Choose the cap generously (e.g. ≥ 64 KiB ≈ 1000+ devices) so real early-scale communities never hit it; it is a safety valve against pathological/malicious vectors.
...
- **`community_channel_log.rs`** — add `WatermarkVector` type (`BTreeMap<String, (u64, u32)>`, canonical CBOR); `ChannelLog::watermark_vector()` folding segments+tail into `{device_id → max (wall_ms,logical)}`; responder-side `collect_events_vector(vector, limit, keep)` alongside the existing scalar `collect_events` (shared inner iteration).
- **`community_channel_log_engine.rs`** — `log_watermark_vector()` accessor; `BackfillQueryRequest` grows `watermark: Option<WatermarkVector>` (additive field); `read_for_query` decrypts/decodes the payload (bounded) and dispatches vector-vs-scalar.
- **`channel_backfill.rs`** — the driver's watermark-reader closure produces the vector; `BackfillLatch` seeding unchanged in shape. **Periodic floor unchanged.**
- **`event_loop.rs`** — requester GET (`:7950`) attaches the encrypted payload when under cap and key available; the dedicated queryable (`:7836`) reads `query.payload()`, runs the cap-before-alloc check, decrypts, decodes, routes to `collect_events_vector`.
- **Tests** — `tests/channel_backfill_integration.rs` gains A.8; a canonical-CBOR pin for `WatermarkVector` in the wire-format fixtures.
