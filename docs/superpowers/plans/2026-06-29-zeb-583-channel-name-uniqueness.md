...
**Design (settled — Approach A, hard-error):** Add an IPC-level name-uniqueness guard in `create_channel_impl`: before inserting the minted `ChannelCreate`, materialize the community's channels and `Err` if a **live** (non-tombstoned) channel already has the same **normalized** name (`trim().to_lowercase()`). Fast-fail exactly like the existing empty-name/length validations.
...
- Placement: after `engine_arc` is resolved (`lib.rs:20147`) and before `insert_local_event` (`20148`), preserving the existing mint/fence/resolve ordering. `name` is moved into `mint_channel_create_event` at `20111`, so capture the normalized name **before** the mint and use it at the check. On the (rare) error path the HLC reservation + mint are wasted — documented-as-fine HLC-burn.
