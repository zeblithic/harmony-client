...
## Execution notes — test-strategy decision (2026-08-06)

Reconnaissance during execution changed where the tests live, versus the sketch above. Recorded here so the coverage is explicit, not silent:

- **`handle_unicast` has no unit tests and no fixture for building a verifiable `CommunityInviteSigned`** (only the full `pkarr_iroh_redeem_full_integration.rs` harness exercises it, via `connectivity_redeem_invite_iroh_inner`). A standalone two-packet `handle_unicast` test (plan Task 2 Step 1) would have required constructing valid signed invite packets for two joiners from scratch — substantial, brittle scaffolding.
- **All enforcement is therefore proven at the engine seam (Task 1), where verifiable events are cheap.** The four `zeb875_*` tests in `community_state_sync.rs` cover: distinct-actor rejection (`InviteAlreadyClaimed { winner }`), same-actor + other-token pass-through, **exactly-one-winner under real multi-thread concurrency** (winner actually inserts a crypto-verified PendingJoin; loser refused), and log-derivation/restart-safety. This is the authoritative correctness proof.
- **The `handle_unicast` wiring** (Task 2) is a direct field pass-through — `signed.invite_token.sig` + the verified `join_event.actor` → the engine seam, with `LocalInsertError::InviteAlreadyClaimed` mapped to the verify error. It is compile-checked and exercised end-to-end by the **existing** `bob_joins_alice_via_iroh_handshake_option_a` e2e, which now flows through `insert_local_claim_bound_pending_join` and validates the wired happy path in the full gate.
- **No new e2e was added (plan Task 3).** A distinct-actor network race needs a second full joiner (~heavy), and a same-actor-retry extension of the existing test would duplicate ~55 lines of redeem setup and break its exact-one-nav-emit assertion — both for marginal coverage beyond the engine proof + the existing e2e. This mirrors ZEB-874's own documented judgment to prove invariants at the logic layer rather than via heavy/racy integration scaffolding. The losing racer's user-facing outcome is the generic `"inviter_unreachable"` (no wire change), per the spec's error-surface section.
