...
This is the per-issue runbook for processing **alpha-feedback intake issues** — GitHub issues that arrive with the `[alpha-feedback]` title prefix, whether or not the `alpha-feedback` label was applied at filing time. Intake issues come primarily from Sub-C's [`FeedbackModal`](../src/lib/components/FeedbackModal.svelte) (the in-app `(?)` → Submit Feedback flow), which prefixes the title but does NOT auto-apply the label (see [§ Filter notes](#filter-notes)), plus manual reports filed through the [issue template](../.github/ISSUE_TEMPLATE/alpha-feedback.md) which DO auto-apply the label and `jenglund` assignee. **Applying the `alpha-feedback` label and the `jenglund` assignee to unlabeled in-app submissions is a triage step**, not a precondition for triage.

**Linear is the authoritative tracker.** GitHub Issues with the `[alpha-feedback]` title prefix (whether or not the `alpha-feedback` label was applied at filing time — see [§ Filter notes](#filter-notes)) are intake-only — every actionable finding is mirrored into a Linear ZEB-NNN ticket as part of the triage workflow below. The GitHub issue stays open as the public-facing receipt; the Linear ticket carries the engineering work. Closing a GitHub issue requires either (a) a linked Linear ticket has been resolved + released, or (b) the disposition is "won't-fix-this-alpha" / "duplicate" / "needs-info-timeout."
