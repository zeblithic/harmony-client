...
**Architecture:** A new `invite_packet: Option<Vec<u8>>` rides inside the sealed `DepositPayload` (and the persisted `DmInboxEntry`, and the in-process `ButlerDepositRequest`). The sender rebuilds+signs the invite from the persisted `Space` record at deposit time (mirroring how it rebuilds the CidNotify). The butler acceptor passes the invite through to the persisted entry untouched (its CidNotify+blob validation is unchanged) behind a size bound. The community relay needs no acceptor change (it holds the sealed blob opaquely). On recover, both entry points (butler `ProdDmInboxIngestCtx::verify` and relay `ProdRelayIngestCtx::ingest_recovered`) **first verify the CidNotify sender against the pristine `OwnerDeviceCache` (`verify_cidnotify_sender_binding`), then** call a shared `apply_deposited_invite` helper — bound to that *verified* sender — to bootstrap **only** the Space, **then** complete admission (`verify_cidnotify_space`). The deposit-recover path never refreshes the `OwnerDeviceCache` from the invite.

> **⚠️ Review-hardening note (CodeRabbit, post-plan):** The recover sequence in Tasks 4–5 below was hardened during PR review. The ORIGINAL plan (and the verbatim code blocks in those tasks) applied the deposited invite *before* `verify_cidnotify_admission` and bound it to the CidNotify's *claimed* `sender_owner_addr`. That ordering was circular: `apply_invite` writes the `OwnerDeviceCache` rows `verify_cidnotify_admission` then reads, so a forged invite + CidNotify could self-verify. The shipped code instead: (1) `verify_cidnotify_sender_binding` against the **pristine** cache, (2) `apply_deposited_invite` pinned to that verified sender (`space_id` / `signing_device_hash` / `inviter_identity_pub`), bootstrapping **only** the Space (never the device cache), (3) `verify_cidnotify_space`. The merged code and the design spec are authoritative wherever they differ from the Task 4–5 snippets.
