...
- **⚠️ REVIEW-FIX ADDENDUM (supersedes the line above): ALL four record types dual-sign.** During PR #463 review, Qodo + CodeRabbit both found that leaving descriptors/reactions *pure-`#2`* breaks on out-of-order delivery: the authority cache is in-memory / non-persisted and Zenoh delivery is unordered, so a pure-`#2` record arriving before its feed's `/authority` hits the legacy `#3` verifier and is dropped. Descriptors and reactions now **also** keep the `#3` signature (like tombstone/follow-list): `#3` binds the claimed `creator_address`/`reactor_address` and is accepted pre-authority; once the feed's authority is cached the `#2` `device_sig` is REQUIRED (`#3`-only rejected), so dual-signing does not weaken revocation. The reactor-address binding is enforced at ingest (`on_reaction_sample` always runs `verify_reaction` first — `verify_reaction_v2` alone cannot tie an enrolled `#2` device to a `#3` node address). Quorum-issued devices therefore keep working on the legacy `#3` path (their feeds simply don't migrate until [ZEB-682]).
...
- [ ] **Step 3: Run it, expect fail** (`sign_descriptor_v2` undefined): `cargo nextest run --locked --features test-fixtures -E 'test(descriptor_v2_sign_verify)'`.
