...
- [ ] **Step 6: Regenerate + pin the hex.** Run the fixture tests once (they panic `REGENERATE …`), paste the printed hex into the `EXPECTED_*_HEX` consts, re-run → PASS. Also run `cargo nextest run -E 'test(zeb375_pex_fixtures)'` → still PASS (byte-pinned 2a bytes unchanged — the wire-compat proof).
