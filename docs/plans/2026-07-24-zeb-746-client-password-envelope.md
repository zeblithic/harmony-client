...
- **Gates (CI-parity, run from `src-tauri/`):** `cargo fmt --all -- --check`; `cargo clippy --locked --all-targets --features test-fixtures --no-deps -- -D warnings`; `cargo nextest run --locked --workspace --all-targets --features test-fixtures`. Because Task 1 changes the `Cargo.toml` dependency graph this session, iterative gates use explicit scoped `-E` selections rather than the module-mapped `scripts/test-select` (which bails after a dependency-graph change); the final pre-PR sweep is always the full `--workspace --all-targets` commands.
