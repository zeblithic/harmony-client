...
> **As-built OUTCOME note (Task 4/5 — the shipped s7 differs from the Task 4 code block below):** running s7 surfaced a real product gap that moves the characterize point EARLIER than the 3 designed boundaries. Co-located SAS pairing + the friend handshake both **work** (`S7 PAIRED`, `S7 FRIENDS`). But the headless **inviter** never persists the paired device's enrollment: after pairing + a reboot, `get_owner_state(P).devices` shows `total=1, peers=0` — B2 is absent — while the **joiner** persists fine (`joiner pairing persisted successfully`). So `set_butler_pin` rejects the device ("not in the enrolled set") and s7 **characterizes at a new "boundary 0" (butler-pin setup)** and PASSES, never reaching HELD/RECV/CLEARED. The shipped s7 therefore (1) does the friend dance **before** the post-pairing relaunch (a deterministic multi-second barrier — a SIGKILL right after `Complete` races/loses the inviter's ~50ms async persist drainer, lib.rs:8821), (2) relaunches P so a *would-be* persisted enrollment is reloaded into the boot-snapshot `fleet_net_enrolled`, and (3) reads the real enrolled device via `get_owner_state` (`deviceVkHex`, owner_commands.rs:118) and characterizes when `peers=0`. The inviter-persist gap is filed as **ZEB-491** (High, Bug) and is an **out-of-scope** follow-up (no `src-tauri` fix in this PR). The code block in Task 4 below is the original design; the file `e2e-harness/tests/e2e_two_node.rs` is authoritative.
