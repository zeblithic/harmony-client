...
> **As-built OUTCOME note (Task 4/5 — the shipped s7 differs from the Task 4 code block below; the file `e2e-harness/tests/e2e_two_node.rs` is authoritative).** s7 **passes** and proves the hard unknowns — co-located SAS pairing + the friend handshake both work (`S7 PAIRED`, `S7 FRIENDS`) — but characterizes EARLIER than the 3 designed boundaries because of a chain of headless multi-device-pairing gaps (all filed as **ZEB-491**, High, Bug; **out of scope** here — no `src-tauri` fix):
>
> 1. **Boundary 0 — intermittent inviter persist.** The headless inviter persists the paired device's enrollment only *intermittently*: after pairing + reboot, `get_owner_state(P).devices` shows `total=1, peers=0` (B2 absent) on some runs and `total=2, peers=1` on others, while the joiner persists reliably (`joiner pairing persisted successfully`). On a `peers=0` run, `set_butler_pin` rejects the device and s7 characterizes here.
> 2. **Boundary 0b — paired joiner doesn't start its engines.** On a `peers=1` run, `set_butler_pin` *succeeds* (`S7 PIN`), but the butler B2 — paired, not minted — boots after relaunch, loads owner-state (`BOOT-PROBE 01`), then stops *before* constructing engines (no `BOOT-PROBE 05: dm-inbox`), so `get_butler_held(B2)` 500s `dm-inbox not running`. s7 characterizes here (likely the cert-only joiner lacking a master seed).
>
> The shipped s7 therefore: does the friend dance **before** the post-pairing relaunch (a multi-second barrier — a SIGKILL right after `Complete` races the inviter's ~50ms async persist drainer, lib.rs:8821); relaunches **P** to rebuild the boot-snapshot `fleet_net_enrolled` (lib.rs:4618→8203); reads the real enrolled device via `get_owner_state.deviceVkHex` (owner_commands.rs:118); relaunches **B2** to start its engines; and characterizes (PASS) at whichever boundary co-located conditions hit. The HELD→RECV→CLEARED chain stays wired so it asserts the moment ZEB-491 lands. The cross-WAN Scenario D3 is the authoritative durability proof.
