...
## Round-1 review revisions (post-implementation, PR #459)

Review (CodeRabbit + Qodo) hardened several behaviors after this plan was written. The step text below reflects the original design; the shipped code differs as follows, and **the code is authoritative**:

- **Merge is commutative + convergent (revises §1.2).** Per-key conflicts resolve to the *smaller* value and each map truncates to its cap in sorted-key order (`union_bounded`), so `merge(A,B) == merge(B,A)`. The request-count cap keeps the lexicographically-smallest `MAX_QUORUM_REQUESTS` ids. Replaces the original "existing entries always win".
- **Declines are cryptographically authenticated (revises §1.1/2.3/2.4/3.1).** `declined_by` is now `BTreeMap<decliner_id_hex, signature_hex>`; a decline only COUNTS (`verified_decliners`) when its signature verifies over a domain-separated `decline_signing_payload` against the decliner's enrolled key AND the decliner is an eligible voter — enrolled, Master-issued, not revoked, and **neither the target nor the initiator** (the device being removed can never veto its own removal; the initiator cancels by letting the request expire). Unverifiable entries are ignored, so a union re-merge of forged junk can't kill a request. The sweep skips a request iff it has ≥1 **verified** decline.
- **Over-horizon expiry rejected (revises §1.2).** The merge drops any request whose `expires_at_ms` exceeds `now + TTL + skew` (and never re-inserts an already-expired one), so a peer can't exhaust the request cap with `u64::MAX`-expiry entries. Arm cells get the same horizon rule.
- **Completion is flush-gated (revises §3.1).** The sweep retires a request from the quorum doc ONLY after `trust_engine.flush_now()` succeeds — never before the revocation is durably persisted. On flush failure the request stays resident and the dirty latch retries. (The IPC write paths keep warn-only flush by design: the replicated doc + dirty latch is their durability boundary, identical to every other FleetSync dataset — only the sweep, which *prunes the retry source*, needs the stronger gate.)
- **Periodic TTL sweep (revises §3.2).** The applied task also ticks a 60 s interval, so expired requests/arms are reclaimed even when no inbound merge nudges the task.
- **Planner caps co-signers (revises §2.1).** When more than `MAX_QUORUM_SIG_ENTRIES` siblings are eligible, the planner keeps the most-recently-live ones before pre-signing, so the request never exceeds the merge's per-map cap (which would otherwise make it unreplicable).
- **View contract (revises §5.2/Task 7).** `QuorumRequestView` also carries `declined` (aggregate: any verified decline). The new `OwnerStateView`/`DeviceView` quorum fields are TypeScript-**optional** (`selfIsMaster?`, `quorumRequests?`, `quorumArmedUntilMs?`, `quorumRemovable?`) — a stale backend omits them; read `quorumRequests ?? []` and the booleans as `=== true`.
- **Deferred:** the repeated ephemeral-runtime FleetSyncEngine-shutdown block in `stop_inner` (12+ pre-existing sites) is a fleet-wide cleanup tracked separately, not folded into this feature PR.
...
**Files:**
- Modify: `src/lib/owner-service.ts` — `OwnerStateView` += `selfIsMaster?: boolean; quorumRequests?: QuorumRequestView[]; quorumArmedUntilMs?: number | null;` `DeviceView` += `quorumRemovable?: boolean;` (all TS-optional — stale-backend tolerance; see "Round-1 review revisions") new `QuorumRequestView` interface; `OwnerService` += `requestQuorumRevocation(deviceVkHex, reason)`, `cosignQuorumRequest(requestId)`, `declineQuorumRequest(requestId)` (invoke + refresh, same shape as `revoke`)
