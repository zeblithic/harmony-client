...
## Implementation notes (deviations + discoveries, 2026-06-10 — kept so this doc matches what shipped)

1. **T2 serde detail:** `Hlc` lacks a `Default` derive, so `FleetNetDoc.pinned_at` uses `#[serde(default = "default_hlc", skip_serializing_if = "hlc_is_unset")]` with a zero-sentinel helper; `FleetNetDoc` gets a manual `Default`. Wire format as planned.
2. **T6 DRY deviation:** the two new datasets' zenoh wiring is one parameterized `spawn_dataset_sync_zenoh_adapter` helper (≈240 duplicated lines avoided), semantics line-for-line the dm-inbox block's; the dm-inbox block itself was deliberately left untouched (+200/−0 in event_loop.rs). `set_outhold` is installed OUTSIDE the iroh-gated block — the hold write is a local CRDT mutation replicated over zenoh, not iroh-dependent. The new engines also got the start_node failure-cleanup carry-out parity the dm-inbox engine has.
3. **T7 plan-gap fix (load-bearing):** `routing_republish` ALSO re-stamps this device's fleet-net self-row (fresh `seen_at` + current relay + `notify_dirty`). Without it every row exceeds the 15-min stale filter ~15 min after boot and sibling secondaries die fleet-wide. No feedback loop: stamp-only changes are excluded from the `selection_view` change-detector (which compares id/endpoint/relay/pin tuples, per D16's "membership, pin, relay").
4. **T7 refresh scope:** identity (case B) re-`enable()` is gated on `publisher.active_handles()` (runtime truth) rather than the boot-time discoverability flag; community (case C) re-registers via `on_community_joined` per known id; friend (case D) via `sync_case_d_handles` over a snapshot taken-then-released under the CRDT lock. **Invite/friend-token (case A) are NOT refreshed** — no clean re-register exists (full payload / consent-expiry-map needed); they ride their epoch schedule, documented in-code. Butler-set resolution reads identity/friend records, so coverage is where it matters.
5. **T8 forward work:** NodeState gained `fleet_net_snapshot`/`fleet_net_device_id`/`fleet_net_enrolled` (needed by the IPC, implied by D17). Pin state reaches the frontend via `get_owner_state` → `build_owner_state_view(pinned_device_id_hex)` → additive `DeviceView.butler_pinned` (`#[serde(default)]`, camelCase via the struct's existing `rename_all`).
6. **T9 harness notes:** A1/A2 DM CAS stores are deliberately separate (the blob-reaches-A2 assertion proves transfer through the outhold row, not store sharing); engines share one fleet CAS for root blobs as production does; owner A's two devices share one minted cert (only A2's deposit path uses cert material). Spec §9's "sibling direct-delivery variant" is covered by the stage-2 CAS-presence assertion + existing serve-queryable tests; the full direct round-trip lands in the manual cross-WAN proof.
7. **D14 amendment** (row-only GC, no CAS deletion) was applied to the spec in the plan commit — see spec D14/§3/§7/§11-2.
