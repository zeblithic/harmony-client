...
| File | Change |
|---|---|
| `src-tauri/src/voice_crypto.rs` | **Add** three v2 community-media helpers (the generic `encrypt_voice_packet`/`decrypt_voice_packet` **stay** — they're shared by presence (`VOICE_PRESENCE_AAD`) and moderation (`VOICE_MODERATION_AAD`); only the two *media* call sites move to v2). Helpers, deliberately keeping **signature-verify separable from decrypt** so the moderation drop runs between them: `seal_and_sign_voice_packet(channel_key, device_sk, community, channel, plaintext)` (sender); `verify_voice_frame_sig(device_vk, community, channel, packet)` (public-key only — strips the 64B sig, Ed25519-verifies the transcript over `nonce ‖ ciphertext`, no channel key needed); `open_voice_packet(channel_key, device_vk, community, channel, packet)` (strips the sig, AEAD-opens `[nonce][ct]` with the v2 AAD). Add `VOICE_PACKET_AAD_V2`, `VOICE_PACKET_SIG_DOMAIN_V2`, `SIG_LEN = 64`, `MIN_PACKET_LEN_V2 = 92` (nonce+tag+sig), a `SigFailed` error variant, and a `…_with_nonce` deterministic seal variant for fixtures. DM/presence/moderation constants + functions unchanged. |
