...
- **`N` and parallel-resolve cost.** `N = COMMUNITY_RELAY_ADVERTISERS_MAX`; a joiner does up to `N` parallel DHT resolves on first contact. Confirm this is within the DHT/bandwidth budget; if not, resolve slots in small batches (slot 0 first, widen on miss).
- **Advertiser-set churn vs. slot stability.** If the advertiser ordering changes (a higher-ranked advertiser appears/leaves), slot assignments shift; ensure the TTL/refresh cadence keeps at least one slot reliably live during reordering, and that a joiner resolving mid-reorder still finds a live beacon (it resolves all slots, so transient gaps are tolerated).
- **Self-promotion convergence.** Confirm the "lowest-ranked eligible online member promotes" rule converges quickly without oscillation when membership/online-state changes concurrently (CRDT-observed online set may differ briefly across members).
