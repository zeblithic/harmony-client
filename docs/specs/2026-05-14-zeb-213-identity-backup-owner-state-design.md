...
- `current_owner_state.last_mutation_wall_ms` is derived at IPC-call time by scanning the live `OwnerState` and taking the maximum `wall_ms` across every HLC-bearing collection: `spaces[]` (`updated_at`), `outbox[]` (`created_at`), `inbox[]` (`received_at`), `markers{}` (`last_read_at`), `owner_device_cache.devices{}` (`learned_at`), `libraries{}` (`added_at` plus `removed_at` when present), and `outbox_tombstones{}` (values are HLCs directly). Missing any of these would cause false-negative staleness for users whose only recent mutations landed in that collection — e.g. "added a library" or "rotated a bound device". O(N) per call where N = total entries; called only when the GUI checks staleness (typically once per app launch + on dismissal expiry). The implementation may opt to cache this as a single derived field on `OwnerState`, refreshed on every write — that's an optimization, not a correctness requirement.
