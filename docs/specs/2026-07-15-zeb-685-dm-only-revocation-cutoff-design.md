...
- **No ongoing friend-message channel exists** (the friend ALPN handshake is one-shot). The ongoing owner→owner path is the **DM tunnel** (`IrohTunnelDmTransport`). Chat DMs get **butler-deposit durability** for offline recipients via the OutboxEntry/message-CID deposit rung — but `RevocationPush` is a **bare control frame that bypasses that rung** (§3.1, §4.4), so its *delivery* is best-effort tunnel-only: an offline friend is not re-delivered to. (Once *received*, the revocation is durably stored + replicated — see §3.4.) The DM layer already carries non-chat **control frames** (`CidNotify`, `Ack`, `DmInvite`) alongside chat content.
...
Add a `DmPacket` variant (wire discriminant `0x05`, next after `CidNotifyWithBlob`=`0x04`) carrying `{ revocation: Box<RevocationCert>, enrollment: Box<EnrollmentCert> }`, encoded as `0x05 || cbor(body)` with its own `encode_packet`/`decode_packet` arm (the `CidNotifyWithBlob` early-return is the length-delimited-variant template). Certs boxed to satisfy `large_enum_variant` (mirrors `inviter_enrollment: Option<Box<EnrollmentCert>>`). It is **not** a chat message (no Space, no message-log entry, never rendered), exactly as `DmInvite` seeds identity without being a chat message.

**No outer frame signature.** Unlike `Invite`/`CidNotify`/`Ack` (which carry `{signed, signature, signed_bytes}` to authenticate the sending #2 device), `RevocationPush` needs none: both certs are already master-signed (self-authenticating), and §3.3 authenticates the *sender* via the tunnel-peer binding + the `revocation.owner == peer's owner` trust-bind. Any of A's devices legitimately relaying A's own master-signed revocation is acceptable, so binding to a specific #2 device adds nothing.

**Delivery = best-effort tunnel (this slice).** The push rides `IrohTunnelDmTransport` best-effort (`mgr.send_dm` to the friend's resolved tunnel targets). It is **not** butler-deposit-durable in this slice: the deposit rung is keyed on `OutboxEntry`/`message_cid`, which a bare control frame is not. So a friend offline at revoke time is not guaranteed to receive it. This is a genuine but bounded limitation (see §4.4) — the cutoff *mechanism* (verify/store/feed) is fully built and durable on B's side; deposit-durable or re-push-on-reconnect delivery is the explicit follow-up increment.

### 3.2 Send side (A revokes device D)

In `revoke_device_inner` (`owner_commands.rs:867`), after `cert_for_feed` is minted and alongside the existing hooks (`:1059`, `:1138`): fetch device D's `EnrollmentCert` from the **trust doc** `trust_snapshot.enrollments.get(&cert_for_feed.target)` (note: `enrollments`/`revocations` are on `harmony_owner::state::OwnerState`, the *trust* doc); enumerate **active** friends from the **CRDT doc** `friend_graph.friends` (filter `status == Active`) — which `revoke_device_inner` must additionally snapshot from `g.sync_engine`/`crdt_state` alongside the handle tuple at `:886-901` (the two `OwnerState` types are distinct; `friend_graph` is NOT on `trust_snapshot`); for each friend, build the `RevocationPush` wire and `mgr.send_dm` to the friend's resolved tunnel targets (mirror `build_bootstrap_invite` + `IrohTunnelDmTransport::send`'s invite-only path). Snapshot the tunnel-manager handle the same way `retire_nudge` is snapshotted. Best-effort per §3.1; no new broadcast primitive — per-friend tunnel send.
...
4. **Offline delivery is best-effort (this slice).** The push is tunnel best-effort (§3.1); a friend offline at revoke time is not guaranteed to receive it. This is strictly better than today (today: no DM-only cutoff at all), and the community path remains belt-and-suspenders when a shared community exists. **Follow-up increment** (file if the security bar demands it): deposit-durable delivery (carry the push through the deposit rung) or re-push A's active revocations on friend reconnect/handshake. The receive/store/feed mechanism this slice builds is unchanged by that follow-up — only the A→B carrier becomes durable.
