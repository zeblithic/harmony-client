...
| …including DMs/vines/storage records | NO — those sign with the device's own #3 node identity; verifiers never consult owner state | Follow-up tickets (§9); copy says "existing direct-message and feed publishing from that device is not blocked yet" |
