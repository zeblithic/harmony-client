...
**Step zero (plan gate) — RESOLVED at plan time (2026-06-10).** The ChannelKey DOES rotate (it is HKDF-derived from the EpochKey, which rotates on Kick/Leave/Ban — `community_channel_log.rs:67-81`, `community_membership.rs:2052-2157`), **but this does not block D20**: the serving side re-seals every backfill reply under its *current* ChannelKey at serve time (`read_for_query` → `list_messages` → `encrypt_channel_packet(channel_key_ref(), ev)`, `community_channel_log_engine.rs:~1517`). Local log storage is epoch-independent plaintext-after-verify, so holders can always serve their full history, and a joiner — who holds the current EpochKey by definition — can decrypt everything served. Forward secrecy is preserved: a removed member lacks the post-rotation EpochKey and cannot decrypt fresh replies. Backfill inherits exactly the live path's key-currency behavior (whatever keeps `channel_key_ref()` current on rotation serves both); no key-history delivery is needed.

**Plan-time amendments to D24 (pagination):** the serving resolver clamps replies to a hard cap (`CHANNEL_BACKFILL_MAX_LIMIT` = 1000 per request). The latch therefore drives a **paging loop**: request → receive a completed page → if the page was full, re-request with `since = max HLC received`; **satisfied = a completed short or empty page**. Reply-stream completion is observable (the zenoh `get` receiver closes when all responders finish). And **triggers 1+2 unify**: a fresh joiner's engine has an empty local log, so `since = max_hlc() = None` falls out of the same engine-start trigger — join needs no special-cased entry point (channels that materialize after the join-time spawn are covered by trigger 3).
