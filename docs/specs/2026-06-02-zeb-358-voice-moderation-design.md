...
**Media drop** (existing media subscriber `harmony/voice/{c}/{ch}/{device}`): before emitting `voice-frame-received`, resolve the sender → owner and drop the frame if that owner `is_muted` or `is_kicked`. To survive the kicked-owner-suppressed-from-roster case, the **presence subscriber keeps a `sender-id → owner` index for every *verified* beacon** (including kicked owners), separate from the *visible* roster, purely for this resolution. The index is keyed by the **media sender-id** — the 16-byte device prefix the media topic/packet uses (per ZEB-351 speaking correlation), derived from the full 32-byte `device` key in the verified beacon — so the lookup matches what the media path actually carries.
