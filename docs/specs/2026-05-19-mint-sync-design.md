...
The floor grows monotonically and is NOT synced — each device tracks its own. Floor entries are dropped only on future cleanup (out of scope for Phase 2); the storage cost is ~80 bytes per ever-deleted account, which is acceptable forever at expected scale.
...
- **D10: Hard-delete + deletion floor for accounts.** Chose hard-delete rather than soft-delete-everything. Accounts are deleted rarely; the deletion floor in `mint_sync_state.cbor` solves the zombie-resurrect problem at ~80 bytes per ever-deleted account. The floor is per-device (not synced) which keeps the design simple — peers learn about a delete by the account simply being absent from the next snapshot they receive from the deleter.
