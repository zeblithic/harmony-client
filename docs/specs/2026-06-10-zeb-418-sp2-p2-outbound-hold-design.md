...
| D14 | Hold lifetime | GC mirrors the matching `OutboxEntry`'s terminal states (Complete / Expired / user-deleted): dataset **row** removed. **Amended at plan time (2026-06-10):** the sibling's CAS copy is retained — it is legitimate DM-history content the sibling's UI references via the replicated InboxEntry (deleting it would evict displayable history; the device would re-fetch the same CID on demand), and no CAS-delete op exists. StorageTier owns blob retention. Only undelivered mail is ever held in the *dataset* |
...
- **GC (as amended, see D14):** when an `OutboxEntry` transitions to Complete/Expired or is deleted via `delete_dm_outbox_entry`, remove the dataset row. CAS copies (originator's and siblings') are untouched — they are the same DM-history blob the replicated InboxEntry references for display; StorageTier owns retention. Row removal is local-then-republished; a stale sibling's merge can resurrect a removed row, which the next status-driven sweep re-deletes (converges because outbox status replicates via OwnerState).
...
| Recipient fetch-back races outhold GC | No race exists post-amendment: GC removes only the dataset row; the serving sibling's CAS copy persists as ordinary history cache |
...
2. ~~CAS GC ledger mechanics~~ — resolved by the D14 amendment: no CAS deletion, no ledger needed.
