...
The result is matched explicitly against `InsertOutcome::Inserted | AlreadyKnown` (success — no teardown), `InsertOutcome::Rejected(verify_err)` (engine rejected — surface as `format!("engine rejected admin bootstrap: {verify_err}")`), and `Err(insert_err)` (engine errored). The two failure paths (`Rejected` and `Err`) tear down via `shutdown_engine_and_cleanup_persistence` — but only when `engine_freshly_created`, the atomic flag returned by `spawn_engine` (PR #90 round-5 — closes a concurrent-redeem race a separate `engine_arc().is_some()` pre-check could not).
...
If steps 3 or 4 fail, Bob aborts redemption and tears down the engine via `shutdown_engine_and_cleanup_persistence` — same rollback path as the existing partial-unicast-send / pending-redemption-timeout cases in ZEB-262. **Re-redemption guard:** the teardown only runs when `spawn_engine` returned `true` (i.e., this call freshly created the engine). On a re-redeem retry where the engine was already running from a prior successful path, `spawn_engine` returns `false` and the rollback skips the teardown so the prior state survives. This mirrors how `spawn_engine` itself is idempotent on re-redemption: the freshly-built channels are dropped and the existing adapter pair stays live. **Atomicity (round-5 fix):** the flag is sourced from `spawn_engine`'s return value rather than a separate pre-`spawn_engine` `engine_arc().is_some()` lookup; sourcing it under the engines-map lock prevents two concurrent redeems from both observing "no existing engine" and racing into a tear-down-the-winner path. The same guard is applied to every rollback site in `redeem_invite_inner`'s invite-only branch (verify-failure, engine-vanish, insert-failure, missing-invite-token, build-packet, encode-packet, no-destinations, all-sends-failed, oneshot-recv-err, timeout, fence-check, apply-rejected) and the OPEN-branch insert paths.
