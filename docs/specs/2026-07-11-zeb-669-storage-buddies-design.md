...
* **ZEB-923 (2026-08-12): PledgeList and BackupSet are leased too.** While non-empty they are re-minted + re-signed + republished hourly (`STORAGE_RECORD_REFRESH_INTERVAL_MS`) by the storage-record publisher task; receivers decay them after `STORAGE_RECORD_TTL_MS` (3 days, keyed on the persisted local receipt clock, boot-grace floored at load). A permanently-dark buddy's pact therefore self-releases via the existing 30 s reconciliation; an empty family stays silent so its receiver rows decay away entirely. Retraction is NOT delayed by the TTL: any mutation that empties a family (removing the last pledge, unflagging the last backup entry) still publishes one signed empty snapshot via the existing on-change triggers, and every boot republishes current — possibly empty — state, so receivers LWW-clear immediately; only the *scheduled renewal* skips empty families.
