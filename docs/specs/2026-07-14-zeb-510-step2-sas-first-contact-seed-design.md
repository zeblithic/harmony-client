...
- `src-tauri/src/fleet_peer_seed.rs`: the doc type.
  ...
  pub struct FleetPeerSeedDoc {
      /// Keyed by the peer's SP1 64-hex device id (same key space as
      /// FleetNetDoc.devices), so a seed and the eventual real FleetNetDoc row
...
```rust
    ...
    // via LWW once it exists — the seed only fills the pre-convergence gap.
    {
        ...
        for (dev_id, row) in &seed_doc.seeds {
            if dev_id == &device_id { continue; } // never seed ourselves
            reachability_resolver.update_with_source(
                self_owner,
                crate::fleet_peer_seed::seed_reachability_payload(row),
                crate::owner_state_types::Hlc { wall_ms: row.observed_at_ms, logical: 0, device_id: dev_id.clone() },
...
5. **Seed HLC stamp:** `wall_ms = observed_at_ms` (pairing time), `device_id = peer's device id`. Because B2's real self-row is re-stamped each boot with a current (later) wall clock, it strictly supersedes. The `lww_newer` tie-break on `device_id` (recon open-q3) is moot here — the seed and real row share the peer's device_id in the HLC, so it reduces to `wall_ms`, and boot-time > pairing-time. ✅ recommend.
