...
4. The freshness-flag (`engine_freshly_created` bool) removed from `spawn_engine` public surface; internalized into guard. ZEB-260 PR #90 round-5 race semantics preserved by design — `spawn_engine_with_guard` only sets `guard.freshly_created = true` if THIS call's `spawn_engine_inner_now` returned `true` (i.e., this caller won the race to insert into the engines map); idempotent-path callers (race losers) get `freshly_created = false` and their guard's Drop is a no-op. The §7.1 `guard_drop_idempotent_call_is_noop` unit test exercises this race semantic at the registry level (the same code path as `redeem_invite_inner`'s race). Runtime verification of the IPC-handler-level race awaits the deferred `redeem_invite_concurrent_race_loser_no_op_on_drop` integration test (per criterion #2's deferral notes).
