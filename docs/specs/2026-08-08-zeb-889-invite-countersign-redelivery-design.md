...
At step "8'. Reserve HLC + mint" (`lib.rs:62816-62844`), compute a wall-clock `cache_now_ms` once, then gate the reserve-HLC + `mint_redemption(...)` block on a cache check: `if let Some(m) = community_registry.get_redemption_mint(token.sig, cache_now_ms).await { m } else { /* reserve join_hlc + mint_redemption(...) */ community_registry.get_or_store_redemption_mint(token.sig, cache_now_ms, m).await }` — using the value **returned** by `get_or_store` (which is the first-writer's mint under a race). On a cache hit no fresh HLC is reserved and no new id is generated — the reused `P1` is retransmitted verbatim.
