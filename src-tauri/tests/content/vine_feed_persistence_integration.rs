...
fn cache_survives_reload() {
    ...
    {
        let mut cache = VineFeedCache::load(Some(&harmony_app::device_dataset_file::test_cipher()), dir.path());
    ...
    let cache2 = VineFeedCache::load(Some(&harmony_app::device_dataset_file::test_cipher()), dir.path());
