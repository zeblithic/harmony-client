...
// ── ZEB-368: iroh as a first-class unicast transport ─────────────────────────
// zenoh 1.9.0 exposes no public/internal API to register a custom transport, so
// this vendored fork teaches zenoh-link's own closed dispatch about the
// `iroh/<hex>` scheme. harmony-app registers the manager factory before
// `zenoh::open`; the closure captures harmony's iroh `Endpoint` +
// `ReachabilityResolver`, so zenoh-link never depends on harmony (no cycle).

/// Locator protocol prefix for the iroh transport: `iroh/<64-hex EndpointId>`.
pub const IROH_LOCATOR_PREFIX: &str = "iroh";

/// Builds the singleton iroh unicast link manager from Zenoh's own
/// `NewLinkChannelSender` (the accept-queue sender the `TransportManager` owns).
pub type IrohLinkManagerFactory =
    std::sync::Arc<dyn Fn(NewLinkChannelSender) -> ZResult<LinkManagerUnicast> + Send + Sync>;

static IROH_LINK_MANAGER_FACTORY: std::sync::OnceLock<IrohLinkManagerFactory> =
    std::sync::OnceLock::new();

/// Register the iroh link-manager factory. Call exactly once, before
/// `zenoh::open`. Errors if called twice.
pub fn register_iroh_link_manager_factory(factory: IrohLinkManagerFactory) -> ZResult<()> {
    if IROH_LINK_MANAGER_FACTORY.set(factory).is_err() {
        bail!("iroh link manager factory already registered");
    }
    Ok(())
}
...
pub enum LinkKind {
    ...
    Vscock,
    Ws,
    /// ZEB-368: iroh QUIC transport (`iroh/<hex>` locators).
    Iroh,
}
    ...
    pub fn new_supported_links<'p>(protocols: impl Iterator<Item = &'p str>) -> Vec<LinkKind> {
        ...
        for p in protocols {
            match p {
                ...
                // ZEB-368: iroh is always supported (no cargo feature gate).
...
impl TryFrom<&Locator> for LinkKind {
    ...
    fn try_from(locator: &Locator) -> Result<Self, Self::Error> {
        ...
        match locator.protocol().as_str() {
            ...
            // ZEB-368: route `iroh/<hex>` to the vendored iroh transport.
...
pub const ALL_SUPPORTED_LINKS: &[LinkKind] = &[
    ...
    #[cfg(all(feature = "transport_vsock", target_os = "linux"))]
    LinkKind::Vscock,
    // ZEB-368: iroh is always in the supported-link set.
    LinkKind::Iroh,
];
...
impl LocatorInspector {
    pub fn is_reliable(&self, locator: &Locator) -> ZResult<bool> {
        ...
        match LinkKind::try_from(locator)? {
            ...
            // ZEB-368: iroh QUIC bidi streams are reliable.
            LinkKind::Iroh => Ok(true),
    ...
    pub async fn is_multicast(&self, locator: &Locator) -> ZResult<bool> {
        ...
        match LinkKind::try_from(locator)? {
            ...
            // ZEB-368: iroh is unicast, never multicast.
            LinkKind::Iroh => Ok(false),
            #[allow(unreachable_patterns)]
            _ => unreachable!(),
        }
    }
...
impl LinkManagerBuilderUnicast {
    pub fn make(
        ...
        match LinkKind::try_from(endpoint)? {
            ...
            // ZEB-368: dispatch to the harmony-registered iroh manager factory.
            LinkKind::Iroh => match IROH_LINK_MANAGER_FACTORY.get() {
                Some(factory) => factory(_manager),
                None => bail!(
                    "iroh link manager factory not registered — call \
                     zenoh_link::register_iroh_link_manager_factory before zenoh::open"
                ),
            },
            #[allow(unreachable_patterns)]
            _ => unreachable!(),
