...
/// Load the relay-optin doc, or — on a corruption/IO error (NOT NotFound) — log
...
/// an empty doc that the next persist writes over the opt-in flags.
pub fn load_doc_or_recover(path: &Path) -> RelayOptInDoc {
    match load(path) {
        Ok(doc) => doc,
        Err(e) => {
            quarantine(path, &e);
...
mod tests {
    ...
    fn load_doc_or_recover_missing_is_default_no_quarantine() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("relay_optin.cbor");
        assert_eq!(load_doc_or_recover(&path), RelayOptInDoc::default());
