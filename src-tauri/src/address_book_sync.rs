    ...
    if matches!(outcome, UpsertOutcome::Inserted | UpsertOutcome::Replaced) {
        match row.entry {
            AddressBookEntry::Reachability(p) => {
                ...
                // ZEB-995: stamp with the record's HLC (wall clamped) so the oracle can
                ...
                // The full HLC (not just wall_ms) keeps wall-millisecond ties on the
                // resolver's exact tie-break instead of delivery order.
                let mut stamp = row.at.clone();
                stamp.wall_ms = stamp.wall_ms.min(
                    now_ms
