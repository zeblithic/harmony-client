...
pub enum LocalInsertError {
    ...
    EngineShuttingDown,
    /// ZEB-875: a claimant-bound single-use invite token already has a
    /// committed PendingJoin from a *different* actor; a distinct claimant is
    /// refused before any state mutation. Returned by the
    /// `NoConflictingClaimantForInvite` precheck under the same `state` lock as
    /// the append, so the claim is atomic — two racing distinct actors can't
...
enum LocalInsertPrecheck {
    ...
    /// ZEB-875: reject if a committed `PendingJoin` for `invite_token_sig`
    /// already exists from an actor other than `claimant`. Binds an invite's
    /// single-use claim to its first actor: a distinct actor is refused, the
    /// same actor is idempotent (falls through to the usual `AlreadyKnown`).
    /// Runs under the same `state` lock as the append, so two concurrent
    /// distinct-actor redeems of one token can't both pass.
    NoConflictingClaimantForInvite {
...
impl LocalInsertPrecheck {
    fn run(&self, state: &CommunityState, admin_addr: OwnerAddr) -> Result<(), LocalInsertError> {
        match self {
            ...
            LocalInsertPrecheck::NoConflictingClaimantForInvite {
                invite_token_sig,
                claimant,
            } => {
                // ZEB-875: scan RAW committed events (not the materialized
                // roster) so the claim is permanent — robust to a later
                // Leave/Kick and to the 30-day PendingJoin materialize expiry.
                // The event log is durably persisted + CRDT-synced, so this
                // verdict survives a host restart with no in-memory index.
                for ev in state.events() {
                    ...
                    } = &ev.kind
                    {
                        if invite_token.sig == *invite_token_sig && ev.actor != *claimant {
                            return Err(LocalInsertError::InviteAlreadyClaimed {
                                winner: ev.actor,
                            });
                        }
                    }
...
impl CommunitySyncEngine {
    ...
    /// `state` lock, the `NoConflictingClaimantForInvite` precheck refuses the
    /// insert if a committed PendingJoin for this invite token's `sig` already
    /// exists from a *different* actor (returns
    /// `LocalInsertError::InviteAlreadyClaimed`); a same-actor re-insert is the
    /// usual idempotent `AlreadyKnown`. This binds an invite's single-use to
    /// its first claimant, so it is enforceable across distinct actors while
    /// the legitimate joiner can always retry. The claim is a pure function of
...
struct InternalCtx {
    ...
    /// `(wire, attempts_left, max_fetched)` back here after its delay;
    /// `internal_task` takes the receiver out of the ctx at start and polls it as
    /// a `select!` arm. ZEB-938: `max_fetched` is the best segment-fetch progress
    /// this frame has reached across prior attempts, carried so `retry_after_miss`
    /// can tell a resuming round (new high-water mark) from a stalled one.
    fetch_retry_tx: mpsc::Sender<(Vec<u8>, u8, usize)>,
    fetch_retry_rx: Option<mpsc::Receiver<(Vec<u8>, u8, usize)>>,
        ...
        tokio::select! {
            ...
            maybe_bytes = ctx.subscriber_rx.recv(), if !inbound_closed => {
                ...
                dispatch_inbound(
                    ...
                    // A fresh frame has no prior progress — the high-water mark
                    // starts at zero (ZEB-938).
                    0,
                    FrameOrigin::Network,
                    &resolved_tx,
                    &resolve_sem,
            ...
            // ZEB-938: `max_fetched` carries the frame's best segment-fetch
            // progress so the resumable budget survives across rounds.
            Some((wire, attempts_left, max_fetched)) = fetch_retry_rx.recv() => {
                dispatch_inbound(
                    &ctx,
                    wire,
                    attempts_left,
...
async fn apply_resolved_ingest(ctx: &InternalCtx, ri: ResolvedIngest) {
    ...
    match result {
        ...
        // ZEB-938: pass the retry budget the progress it made (`fetched`) and its
        // best-so-far (`max_fetched`) so the budget resumes rather than restarts.
        ResolveResult::Miss { wire, fetched } => {
            schedule_fetch_retry(ctx, wire, attempts_left, max_fetched, fetched).await
        }
...
enum RetryStep {
    /// The budget is spent after a run of no-progress rounds — drop the frame
    /// and report degraded. `retry_after_miss` returns this only when the frame
    /// made no new progress AND `attempts_left` has counted down to zero.
    Exhausted,
    /// Re-inject the frame for another attempt, carrying the remaining budget
    /// and the best-progress-so-far high-water mark.
    Retry {
        attempts_left: u8,
        max_fetched: usize,
    },
...
/// reached. A miss that set a NEW high-water mark (`this_fetched >
/// prior_max_fetched`) is *progress*: reset the no-progress budget to full, so
/// the frame keeps retrying as long as each round eventually fetches at least
/// one new segment. Otherwise the budget counts down and terminates at zero.
///
/// This is what turns the former all-or-nothing segmented-root fetch (one
/// missing segment abandoned the whole root after a fixed `1 +
/// FETCH_RETRY_ATTEMPTS` attempts) into a resumable one: a large root converges
/// on a lossy path because the CAS caches every fetched segment, so a retry only
/// network-fetches the still-missing ones and each round can advance the mark.
/// Only a *stall* — no new segment for `FETCH_RETRY_ATTEMPTS + 1` consecutive
/// rounds, i.e. a genuinely-unfetchable segment — abandons the root.
///
/// The all-or-nothing case (`this_fetched == prior_max_fetched == 0`: a
/// monolithic root, or a manifest blob that itself never fetches) never makes
/// progress, so it reduces to exactly the pre-ZEB-938 fixed budget — that
/// path's behavior is unchanged. Total attempts stay bounded: progress can
/// occur at most `MANIFEST_SEGMENT_CAP` times (each strictly raises the mark),
/// with at most `FETCH_RETRY_ATTEMPTS + 1` no-progress rounds between, so the
/// re-injection chain cannot run forever.
fn retry_after_miss(attempts_left: u8, prior_max_fetched: usize, this_fetched: usize) -> RetryStep {
    if this_fetched > prior_max_fetched {
        RetryStep::Retry {
            attempts_left: FETCH_RETRY_ATTEMPTS,
            max_fetched: this_fetched,
        }
...
async fn schedule_fetch_retry(
    ...
    {
        let (next_attempts_left, new_max_fetched) = match retry_after_miss(
            attempts_left,
            prior_max_fetched,
            this_fetched,
        ) {
            RetryStep::Exhausted => {
                // Budget spent after a run of no-progress rounds. NOW it is a
                // terminal drop, and reports as the BlobNotFound this path
                // used to return on the very first miss.
                ctx.sync_stats
                ...
                report_degraded(
                    ...
                    "blob_not_found",
                    format!(
                        "state-root blob not fetchable within {}ms after {} no-progress attempts",
            ...
            RetryStep::Retry {
                attempts_left,
                max_fetched,
            } => (attempts_left, max_fetched),
        };

        // ZEB-938: a round that reached a new high-water mark reset the budget —
        // a large root is resuming across rounds. Count it as the fleet-
        // diagnostic complement to `fetch_retries_exhausted`.
        if this_fetched > prior_max_fetched {
            ctx.sync_stats
        ...
        tracing::info!(
            community_id = ?ctx.community_id,
            attempts_left = next_attempts_left,
        ...
        tokio::spawn(async move {
            ...
            // CAS fetch.
            if tx
                .try_send((wire, next_attempts_left, new_max_fetched))
                .is_err()
            {
...
mod tests {
    ...
    /// segments (progress) resets the no-progress budget to full — even at
    /// `attempts_left == 0`. This is what lets a large segmented root converge
    /// on a lossy path across arbitrarily many rounds, as long as each round
    /// eventually fetches at least one new segment (the CAS caches every fetched
    /// segment, so re-fetches of earlier ones are local cache hits).
    #[test]
    fn retry_after_miss_resets_budget_on_progress() {
        assert_eq!(
            retry_after_miss(0, 3, 4),
            RetryStep::Retry {
                attempts_left: FETCH_RETRY_ATTEMPTS,
                max_fetched: 4,
            },
            "a new high-water mark resets the budget even with no attempts left"
        );
        assert_eq!(
            retry_after_miss(FETCH_RETRY_ATTEMPTS, 0, 1),
            RetryStep::Retry {
                attempts_left: FETCH_RETRY_ATTEMPTS,
                max_fetched: 1,
            },
    ...
    /// ZEB-938: a miss that fetched no NEW segment (`this <= prior`) makes no
    /// progress — the budget counts down and carries the existing high-water
    /// mark forward. A regression (`this < prior`, e.g. an earlier segment
    /// evicted mid-chain) is treated identically: it is not progress.
    #[test]
    fn retry_after_miss_counts_down_without_progress() {
        assert_eq!(
            retry_after_miss(3, 5, 5),
            RetryStep::Retry {
                attempts_left: 2,
                max_fetched: 5,
            },
            "re-reaching the same mark decrements and keeps the mark"
        );
        assert_eq!(
            retry_after_miss(1, 5, 5),
            RetryStep::Retry {
                attempts_left: 0,
                max_fetched: 5,
            },
    ...
    /// ZEB-938: the budget still terminates. `attempts_left == 0` with no new
    /// progress abandons the root and reports degraded (via the caller). The
    /// all-or-nothing monolithic case (`this == prior == 0`) reduces to exactly
    /// the pre-ZEB-938 fixed budget, so that path's behavior is unchanged.
    #[test]
    fn retry_after_miss_exhausts_after_a_run_of_no_progress() {
        assert_eq!(retry_after_miss(0, 5, 5), RetryStep::Exhausted);
        assert_eq!(
            retry_after_miss(0, 0, 0),
            RetryStep::Exhausted,
            "monolithic all-or-nothing: no progress signal, unchanged budget"
        );
    }
    ...
    // ── ZEB-875: claimant-bound atomic single-use invite claim ───────────
    //
    // The claim is a `NoConflictingClaimantForInvite` precheck run under the
    // same `state` lock as the append: the first committed PendingJoin for an
    // invite token's sig fixes the claimant; a *distinct* actor is refused, the
    // same actor is idempotent. Tests 1–3 prove the precheck logic
    // deterministically (the prior claim is staged straight into the log, so
    // the rejection fires before any verify — no crypto needed); test 4 proves
    // exactly-one-winner atomicity through the real verifying insert.

    fn zeb875_stub_token(
        inviter: OwnerAddr,
        sig: [u8; 64],
    ...
    fn zeb875_pending_join(
        community_id: SpaceId,
        joiner: &crate::community_membership::TestOwner,
        ...
        SignedMembershipEvent {
            ...
            ..sign_event(&payload, &joiner.device_key).expect("sign pending join")
        }
    }

    /// A distinct actor redeeming the same invite token sig is refused by the
    /// claim precheck, before any insert.
    #[tokio::test]
    async fn zeb875_claim_rejects_distinct_actor_on_same_token() {
        use crate::community_membership::mint_test_owner;
        let alice = mint_test_owner(0xAA);
        let bob = mint_test_owner(0xBE);
        let carol = mint_test_owner(0xCA);
        let community_id = SpaceId([0x75; 16]);
        let dir = tempfile::tempdir().expect("tempdir");
        let (engine, _sub_tx) = closing_guard_engine(&dir, community_id, &alice);

        let sig = [0x33u8; 64];
        let token = zeb875_stub_token(alice.owner, sig);
        // Stage Bob's claim directly — the precheck reads the committed log
        // regardless of how the event got there.
        {
            let state = engine.state();
            let mut g = state.lock().await;
            g.insert_verified_for_test(zeb875_pending_join(
        ...
        assert!(
            ...
            "a distinct actor must be refused with the first claimant as winner, got: {res:?}"
        );
    }
    ...
    /// Exactly one of two concurrent distinct-actor claims on one invite token
    /// wins the insert; the other is refused. Structure ported from
    /// `pkarr_invite_publisher::try_consume_friend_token_exactly_one_winner`.
    #[tokio::test(flavor = "multi_thread", worker_threads = 4)]
    async fn zeb875_claim_exactly_one_winner_under_concurrency() {
        use crate::community_membership::mint_test_owner;
        use crate::community_state_crdt::InsertOutcome;
        let alice = mint_test_owner(0xAA);
        let bob = mint_test_owner(0xBE);
        ...
        let dir = tempfile::tempdir().expect("tempdir");
        let (engine, _sub_tx) = closing_guard_engine(&dir, community_id, &alice);
        // Alice Joined so her enrolled key verifies the invite-token sig (P5).
        engine
            .insert_local_event(closing_guard_bootstrap_join(
                community_id,
                &alice,
        ...
        let won_a = matches!(ra, Ok(InsertOutcome::Inserted));
        let won_b = matches!(rb, Ok(InsertOutcome::Inserted));
        assert!(
            won_a ^ won_b,
            "exactly one distinct-actor claim must win the race: a={ra:?} b={rb:?}"
        );
        let loser_claim_rejected = matches!(ra, Err(LocalInsertError::InviteAlreadyClaimed { .. }))
            || matches!(rb, Err(LocalInsertError::InviteAlreadyClaimed { .. }));
        assert!(
