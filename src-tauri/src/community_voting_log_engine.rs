...
use crate::community_voting_core::{PollEventKindCode, SignedVotingEvent, Tier};
...
impl<R: tauri::Runtime> VotingLogEngine<R> {
    ...
    async fn maybe_trigger_beacon_for_tier3_create(&self, event: &SignedVotingEvent) {
        ...
        // Derive the beacon seed from the poll's event hash + community_epoch.
        // We need community_epoch from the DfrostLogRegistry (if wired).
        let community_epoch = {
        ...
        // Derive poll_create_event_hash from the event's signing bytes.
        let poll_create_event_hash = match event.signing_bytes() {
            Ok(sb) => {
    ...
    pub async fn publish_event(&self, event: SignedVotingEvent) -> Result<(), String> {
        ...
        // (3a) Task 10: after a Tier 3 PollCreate is applied, trigger a VRF
        // beacon request so sortition can proceed. Fire-and-forget; errors
        // are logged inside maybe_trigger_beacon_for_tier3_create.
