...
mod zeb846_accessor_ceiling {
    ...
    fn poison_survives_reload_but_is_still_deferred_on_materialize() {
        ...
        community_state_persist::save_crdt(&crate::device_dataset_file::test_cipher(), &path, &state).unwrap();
        let reloaded = community_state_persist::load_crdt(&crate::device_dataset_file::test_cipher(), &path, COMMUNITY).unwrap();
