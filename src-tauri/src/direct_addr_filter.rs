...
/// the interface is operationally up (`if_addrs::IfOperStatus::Up`). A physical,
/// non-virtual interface can still list a stale address while operationally **down**
/// (cable pulled, Wi-Fi dropped) — `oper_up` lets the allowlist reject those so a
/// peer never dials an unreachable endpoint. (Cursor Bugbot, PR #213.)
#[derive(Debug, Clone)]
...
pub fn routable_direct_addresses(
    ...
    iroh_addrs
        ...
        .filter(|sa| {
            ...
            local_ifaces
                .iter()
                .any(|iface| iface.ip == ip && iface.oper_up && !is_virtual_iface(&iface.name))
        })
        .collect()
}
...
///
/// **Blocking:** `get_if_addrs()` is a synchronous OS syscall (`getifaddrs(3)` /
/// `GetAdaptersAddresses`). Async callers on a Tokio worker should prefer
/// [`gather_routable_direct_addrs_async`], which offloads the syscall to a
/// blocking thread. This sync entry point is retained for sync `Fn() -> Vec`
/// callers — the pkarr routing-blob builder — that cannot `.await`.
...
/// Async wrapper over [`gather_routable_direct_addrs`] for callers on a Tokio
/// worker. The interface enumeration is a blocking syscall, so a burst of
/// reachability republishes (VPN connect, sleep/wake, dock/undock fire `if-watch`
/// change events back-to-back) could otherwise stall an executor worker. We run it
/// on the blocking pool via `spawn_blocking`. On task panic / runtime shutdown we
/// return an empty set — peers fall back to the home relay, exactly as in the
/// all-filtered case. (Qodo / CodeAnt / CodeRabbit, PR #213.)
pub async fn gather_routable_direct_addrs_async(iroh_addrs: Vec<SocketAddr>) -> Vec<SocketAddr> {
    match tokio::task::spawn_blocking(move || gather_routable_direct_addrs(iroh_addrs)).await {
        Ok(addrs) => addrs,
        Err(e) => {
            tracing::warn!(
                "direct-addr filter task failed ({e}); falling back to home relay (empty direct set)"
            );
            Vec::new()
        }
    }
...
mod tests {
    ...
    fn iface(ip: &str, name: &str) -> LocalIface {
        iface_status(ip, name, true)
    }
    fn iface_status(ip: &str, name: &str, oper_up: bool) -> LocalIface {
        LocalIface {
            ip: ip.parse().unwrap(),
            name: name.to_string(),
            oper_up,
    ...
    #[test]
    fn drops_down_non_virtual_interface() {
        // A physical interface can still list an address while operationally down
        // (cable pulled / Wi-Fi dropped). `oper_up == false` must reject it — a
        // peer would otherwise dial an unreachable endpoint. (Cursor Bugbot #213.)
        let ifaces = vec![
            iface_status("192.168.86.44", "en0", false), // down — must be dropped
            iface_status("10.0.0.7", "eth0", true),      // up   — must be kept
        ];
        let got =
            routable_direct_addresses(&[sa("192.168.86.44", 5000), sa("10.0.0.7", 5000)], &ifaces);
        assert_eq!(got, vec![sa("10.0.0.7", 5000)]);
    }
